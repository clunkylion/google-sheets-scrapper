...
export const uploadSimpleBilledReportToGoogleSheet = async () => {
  try {
    ...
    if (validateDuplicatesValues && validateDuplicatesValues.length > 0) {
      ...
      return {
        success: true,
        message: 'Simple Billed report uploaded successfully',
      };
    }

    return {
      success: true,
      message: 'Simple Billed report already uploaded',
    };
  } catch (error) {
    console.log(error);
    return {
...
(async () => {
  console.log(await uploadSimpleBilledReportToGoogleSheet());
})();
